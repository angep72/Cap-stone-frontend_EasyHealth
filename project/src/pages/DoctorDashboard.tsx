...
export function DoctorDashboard() {
  ...
  const approvedAppointments = useMemo(
    () => appointments.filter((appointment) => normalizeStatus(appointment.status) === 'approved'),
    [appointments]
  );
  const completedAppointments = useMemo(
    () => appointments.filter((appointment) => normalizeStatus(appointment.status) === 'completed'),
    [appointments]
  );
  ...
  useEffect(() => {
    if (doctorProfile) {
      buildChartData(appointments, prescriptions, labTestRequests, consultations);
    }
  }, [appointments, prescriptions, labTestRequests, consultations, doctorProfile]);
  ...
  const calculateStats = (appointments: Appointment[]) => {
    const approved = appointments.filter((a) => normalizeStatus(a.status) === 'approved').length;
  ...
  const fetchDoctorConsultations = async () => {
    ...
    try {
      ...
      const data = (await api.getConsultations()) as any[];
      const doctorConsultations = data.filter((consult: any) => {
        const docId = typeof consult.doctor_id === 'object' ? consult.doctor_id?._id : consult.doctor_id;
        return docId === doctorId;
      });
  ...
  const buildChartData = (
    ...
    console.log('DoctorDashboard: raw appointments', appointmentsData);
    console.log('DoctorDashboard: raw consultations', consultationsData);
    const now = new Date();
    const startOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const thirtyDaysAgo = new Date(startOfToday);
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 29);

    const recentConsultations = consultationsData
      .map((consult: any) => {
        const status = normalizeStatus(consult.status);
        const consultDate =
          parseDateValue(consult.consultation_date) ||
          parseDateValue(consult.consultationDate) ||
          parseDateValue(consult.created_at) ||
          parseDateValue(consult.createdAt) ||
          parseDateValue(consult.updated_at) ||
          parseDateValue(consult.updatedAt);
      ...
      .filter((entry) => {
        if (!entry.consultDate) {
          console.log('DoctorDashboard: skipped consultation (no date)', entry.raw?._id || entry.raw?.id);
          return false;
        }

        const inRange = entry.consultDate >= thirtyDaysAgo && entry.consultDate <= now;
        if (!inRange) {
          console.log('DoctorDashboard: skipped consultation (out of range)', {
            id: entry.raw?._id || entry.raw?.id,
            date: entry.consultDate.toISOString(),
          });
        } else {
          console.log('DoctorDashboard: counted consultation', {
            id: entry.raw?._id || entry.raw?.id,
    ...
    recentConsultations.forEach(({ consultDate }) => {
    ...
    const dailyConsultations = Object.entries(dailyCounts).map(([date, values]) => {
      console.log('DoctorDashboard: daily totals', {
        date,
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const consultationCount = recentConsultations.filter(({ consultDate }) => {
        return consultDate && consultDate >= weekStart && consultDate <= weekEnd;
      }).length;
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <Card title="Recently Completed Consultations">
          {loading ? (
            <div className="flex items-center justify-center py-8">
              <Loader />
            </div>
          ) : completedAppointments.length === 0 ? (
            <p className="text-center text-gray-600 py-8">No completed consultations yet</p>
          ) : (
            <div className="space-y-4">
              {completedAppointments.slice(0, 6).map((apt) => (
                <div
                  key={apt.id}
